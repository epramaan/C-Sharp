...
namespace OIDC_DOT_NET_INTEGRATION_PRODUCTION.Controllers
{
    public class EpramaanController : Controller
    {
        ...
        //staging
        public static readonly string client_id = "1*******7";
        public static readonly string salt = "1****1";
        public static readonly string aeskey = "c*************************1b3";
        public static readonly string redirect_uri = "http://localhost:44355/Epramaan/ProcessAuthCodeAndGetToken";
        public static readonly string Certificate = "D:/Integration/aspDotNet/OidcDotNetProduction/OidcDotNetProduction/epramaan_staging.crt";
        ...
        [HttpPost]
        public ActionResult createRequestForLogoutOnEpramaan(string sessionId, string sub) {
            string logoutRequestId = Guid.NewGuid().ToString();
            string inputValue = client_id + sessionId + iss + aeskey + sub + service_logout_uri;
            string hmac = hashHMACHex(inputValue, aeskey);
            string data = "{" + "\"clientId\":\"" + client_id + "\"," + "\"sessionId\":\"" + sessionId + "\"," + "\"hmac\":\"" + hmac + "\"," + "\"iss\":\"" + iss + "\"," + "\"logoutRequestId\":\"" + logoutRequestId + "\"," + "\"sub\":\"" + sub + "\"," + "\"redirectUrl\":\"" + service_logout_uri + "\"," + "\"customParameter\":\"" + customParameter + "\"" + "}";
            ViewBag.data = data;
            ViewBag.logout_uri = logout_uri;
            return View();
        }

        [HttpGet]
        public ActionResult LogoutOnEpramaan(string LogoutResponse)         //logout url in epramaan db
        {
            byte[] bytes = Convert.FromBase64String(LogoutResponse);
            string decodedString = Encoding.UTF8.GetString(bytes);
            JsonDocument jsonDocument = JsonDocument.Parse(decodedString);
            JsonElement root = jsonDocument.RootElement;
            string logoutStatus = root.GetProperty("logoutStatus").ToString();
            ViewBag.logoutStatus = logoutStatus;
            return RedirectToAction("LoginUsingEpramaan", "Epramaan", new { logoutStatus = logoutStatus });
        }
